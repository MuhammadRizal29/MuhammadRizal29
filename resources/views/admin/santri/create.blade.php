...
<div class="container mx-auto px-4 sm:px-8">
    <div class="py-8">
        <h2 class="text-2xl font-semibold leading-tight">Tambah Santri Baru</h2>
        <form action="{{ route('admin.santri.store') }}" method="POST" enctype="multipart/form-data" class="mt-4">
            @csrf
            <div class="mb-4">
                <label for="c_santri" class="block text-gray-700 text-sm font-bold mb-2">Kode Santri:</label>
                <input type="text" name="c_santri" id="c_santri" value="{{ $c_santri }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('c_santri') border-red-500 @enderror" readonly>
                @error('c_santri')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nis" id="nis" value="{{ old('nis') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('nis') border-red-500 @enderror" required>
                @error('nis')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="tempat_lahir" id="tempat_lahir" value="{{ old('tempat_lahir') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('tempat_lahir') border-red-500 @enderror" required>
                @error('tempat_lahir')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="date" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('tanggal_lahir') border-red-500 @enderror" required>
                @error('tanggal_lahir')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="c_kelas" id="c_kelas" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('c_kelas') border-red-500 @enderror" required>
                    ...
                    @foreach($kelas as $k)
                    <option value="{{ $k->c_kelas }}" {{ old('c_kelas') == $k->c_kelas ? 'selected' : '' }}>{{ $k->nama_kelas }}</option>
                    @endforeach
                </select>
                @error('c_kelas')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <select name="c_orangtua" id="c_orangtua" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('c_orangtua') border-red-500 @enderror" required>
                    <option value="" disabled selected>Pilih Orang Tua/Wali</option>
                    @foreach($orangtua as $o)
                    <option value="{{ $o->c_orangtua }}" {{ old('c_orangtua') == $o->c_orangtua ? 'selected' : '' }}>{{ $o->nama }}</option>
                    @endforeach
                </select>
                @error('c_orangtua')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
